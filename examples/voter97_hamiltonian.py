# -*- coding: utf-8 -*-
"""
Created on Wed Jul 21 12:26:58 2021
...
#For uncoupled system, d_1 = parameters[2]  = 0
...
def grad_pot_voter97(x, parameters):
    ...
    dVdy = parameters[2]*np.cos(2*pi*x[0]) + 2*parameters[3]*x[1]
...
def guess_lin_voter97(eqPt, Ax, parameters):
    ...
    """    
    correcx, correcy = eigvector_voter97(parameters)
...
def jacobian_voter97(eqPt, parameters):
    ...
    dVdy = parameters[2]*np.cos(2*pi*x) + 2*parameters[3]*y
...
def diffcorr_acc_corr_voter97(coords, phi_t1, x0, parameters):
    ...
    dVdy = parameters[0]*np.cos(2*pi*x1) + 2*parameters[3]*y1
    vxdot1 = -dVdx
    vydot1 = -dVdy
